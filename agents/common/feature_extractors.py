...
class TrackingClassifyingContourExtractor(object):
    ...
    def __call__(self, state, action):
        ...
        if self.count%100 ==0:
            ...
            output_filepath = os.path.join(FEATURES_DIR, "features.pkl".format(self.count))
...
class BasicFeature(object):
    ...
    def calcFeatures(self, state, action):
        ...
        for i in range(16):
            for j in range(14):
                ...
                for r in range(10):
                    for c in range(15):
                        #print screen.shape
                        #print "screen", i,j,r,c,10 * i + r, 15 * j + c, screen[10 * i + r][15 * j + c]
                        #print "index", i * 14 * 128 + j * 128 + screen[10 * i + r][15 * j + c]
                        self.features[i * 14 * 128 + j * 128 + screen[(10 * i + r) * 210 + 15 * j + c]] = 1
                        #found.append(screen[(10 * i + r) * 210 + 15 * j + c])
                #if self.count % 100 == 0:
                #    print i,j,found
        ...
        #self.features = features#.iteritems()
